import tkinter as tk
from tkinter import ttk, simpledialog, messagebox
import matplotlib.pyplot as plt
from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg
import numpy as np

global_points = []

class InteractiveLineChart:
    def __init__(self, root):
        self.root = root
        self.root.title("RawAccel Alternative")

        # Use GTK theme if available (cross-platform)
        try:
            ttk.Style().theme_use('clam')  # Good cross-platform theme
        except tk.TclError:
            print("Clam theme not available. Using default.")

        self.num_subdivisions = 10
        self.num_points = self.num_subdivisions + 1
        self.x_values = np.linspace(0, 1, self.num_points)
        self.points = list(np.linspace(0, 1, self.num_points))

        global global_points
        global_points = self.points[:]

        self.fig, self.ax = plt.subplots(figsize=(6, 4))
        self.line, = self.ax.plot(self.x_values, self.points, marker='o', linestyle='-', markersize=8)
        self.ax.set_xlim(0, 1)
        self.ax.set_ylim(0, 1)
        self.ax.set_xticks(np.linspace(0, 1, self.num_subdivisions + 1))
        self.ax.set_yticks(np.linspace(0, 1, self.num_subdivisions + 1))
        self.ax.grid(True, linestyle='--', alpha=0.7)
        self.ax.set_aspect('equal')
        self.fig.patch.set_facecolor('#f0f0f0')

        self.canvas = FigureCanvasTkAgg(self.fig, master=root)
        self.canvas.draw()
        self.canvas.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=1, padx=10, pady=10)

        self.canvas.mpl_connect('button_press_event', self.on_press)
        self.canvas.mpl_connect('motion_notify_event', self.on_motion)
        self.canvas.mpl_connect('button_release_event', self.on_release)

        self.dragging_point = None

        # Main frame for controls
        controls_frame = ttk.Frame(root, padding=10)
        controls_frame.pack(side=tk.TOP, fill=tk.X)

        # Preset dropdown
        self.preset_label = ttk.Label(controls_frame, text="Presets:")
        self.preset_label.grid(row=0, column=0, padx=5, pady=5, sticky='w')
        self.presets = ["Linear", "Natural", "Power", "Ease In Out", "Ease In", "Ease Out", "Sine", "Custom",
                        "Expo", "Log", "Sqrt", "Cubic", "Quintic", "Circular In", "Circular Out",
                        "Circular In Out", "Ease In Sine", "Ease Out Sine", "Ease In Out Sine",
                        "Ease In Quad", "Ease Out Quad", "Ease In Out Quad"]
        self.preset_var = tk.StringVar()
        self.preset_var.set(self.presets[0])
        self.preset_dropdown = ttk.Combobox(controls_frame, textvariable=self.preset_var, values=self.presets, state="readonly")
        self.preset_dropdown.grid(row=0, column=1, padx=5, pady=5, sticky='we')
        self.preset_dropdown.bind("<<ComboboxSelected>>", self.apply_preset)

        # Subdivision textbox
        self.subdivision_label = ttk.Label(controls_frame, text="Subdivisions:")
        self.subdivision_label.grid(row=1, column=0, padx=5, pady=5, sticky='w')
        self.subdivision_entry = ttk.Entry(controls_frame, width=10)
        self.subdivision_entry.insert(0, str(self.num_subdivisions))
        self.subdivision_entry.grid(row=1, column=1, padx=5, pady=5, sticky='w')
        self.update_button = ttk.Button(controls_frame, text="Update", command=self.update_subdivisions)
        self.update_button.grid(row=1, column=2, padx=5, pady=5, sticky='w')

        # Custom curve input
        self.custom_curve_button = ttk.Button(controls_frame, text="Enter Custom Curve", command=self.enter_custom_curve)
        self.custom_curve_button.grid(row=2, column=0, columnspan=3, padx=5, pady=5, sticky='we')

        # Reset and Apply buttons
        button_frame = ttk.Frame(controls_frame)
        button_frame.grid(row=3, column=0, columnspan=3, padx=5, pady=5, sticky='we')
        self.reset_button = ttk.Button(button_frame, text="Reset", command=self.reset_curve)
        self.reset_button.pack(side=tk.LEFT, padx=5)
        self.apply_button = ttk.Button(button_frame, text="Apply", command=self.apply_changes)
        self.apply_button.pack(side=tk.LEFT, padx=5)

        # Status label
        self.status_label = ttk.Label(root, text="Ready", anchor=tk.W)
        self.status_label.pack(side=tk.BOTTOM, fill=tk.X, padx=10, pady=5)

        # Output Text
        self.output_label = ttk.Label(root, text="Output:")
        self.output_label.pack(padx=10, pady=(0, 0), anchor=tk.W)
        self.output_text = tk.Text(root, height=4, width=40, font=('Consolas', 10),
                                  background='#e0e0e0', foreground='#101010',
                                  insertbackground='#101010')
        self.output_text.pack(padx=10, pady=(0, 10), fill=tk.X)

        controls_frame.columnconfigure(1, weight=1)



    def apply_changes(self):
        """Placeholder."""
        global global_points
        # self.output_text.delete(1.0, tk.END)
        # self.output_text.insert(tk.END, f"{global_points}")
        # self.status_label.config(text="Changes Applied (Placeholder)", foreground='green')
        step = self.num_subdivisions
        output_string = "accel_profile = custom "
        output_string += f"{step+1:d} "  # Use the calculated step
        output_string += " ".join(f"{p:.3f}" for p in self.points) #format the points
        self.output_text.delete(1.0, tk.END)
        self.output_text.insert(tk.END, output_string)


    def reset_curve(self):
        """Resets the curve."""
        self.points = list(np.linspace(0, 1, self.num_points))
        global global_points
        global_points = self.points[:]
        self.line.set_ydata(self.points)
        self.canvas.draw()
        self.preset_var.set("Linear")
        self.status_label.config(text="Curve Reset", foreground='blue')

    def apply_preset(self, event=None):
        """Applies preset curves."""
        preset = self.preset_var.get()
        x = np.linspace(0, 1, self.num_points)
        if preset == "Linear":
            self.points = list(x)
        elif preset == "Natural":
            self.points = list(x**2)
        elif preset == "Power":
            self.points = list(x**3)
        elif preset == "Ease In Out":
            self.points = list((np.sin((x * np.pi) - np.pi / 2) + 1) / 2)
        elif preset == "Ease In":
            self.points = list(1 - np.cos((x * np.pi) / 2))
        elif preset == "Ease Out":
            self.points = list(np.sin((x * np.pi) / 2))
        elif preset == "Sine":
            self.points = list(np.sin(x * np.pi))
        elif preset == "Expo":
            self.points = list(2**(10 * (x - 1)))
        elif preset == "Log":
            self.points = list((np.log10(1 + 9 * x)))
        elif preset == "Sqrt":
            self.points = list(np.sqrt(x))
        elif preset == "Cubic":
            self.points = list(x**3)
        elif preset == "Quintic":
            self.points = list(x**5)
        elif preset == "Circular In":
            self.points = list(1 - np.sqrt(1 - x**2))
        elif preset == "Circular Out":
            self.points = list(np.sqrt(1 - (x - 1)**2))
        elif preset == "Circular In Out":
            self.points = [(1 - np.sqrt(1 - (2 * val - 1)**2)) / 2 if val < 0.5 else (np.sqrt(1 - (2 * val - 1)**2) + 1) / 2 for val in x]
        elif preset == "Ease In Sine":
            self.points = list(1 - np.cos(x * np.pi / 2))
        elif preset == "Ease Out Sine":
            self.points = list(np.sin(x * np.pi / 2))
        elif preset == "Ease In Out Sine":
            self.points = list(-(np.cos(np.pi * x) - 1) / 2)
        elif preset == "Ease In Quad":
            self.points = list(x**2)
        elif preset == "Ease Out Quad":
            self.points = list(1 - (1 - x) ** 2)
        elif preset == "Ease In Out Quad":
            self.points = [2 * val**2 if val < 0.5 else 1 - (2 * (1 - val))**2 / 2 for val in x]
        elif preset == "Custom":
            return

        global global_points
        global_points = self.points[:]

        self.line.set_ydata(self.points)
        self.canvas.draw()
        self.status_label.config(text=f"Preset Applied: {preset}", foreground='green')

    def update_subdivisions(self):
        """Updates subdivisions."""
        try:
            value = int(self.subdivision_entry.get())
            if 5 <= value <= 300:
                self.num_subdivisions = value
                self.num_points = self.num_subdivisions + 1
                self.grid_points = np.linspace(0, 1, self.num_subdivisions + 1)
                self.x_values = np.linspace(0, 1, self.num_points)
                self.points = list(np.linspace(0, 1, self.num_points))
                global global_points
                global_points = self.points[:]

                self.ax.clear()
                self.line, = self.ax.plot(self.x_values, self.points, marker='o', linestyle='-', markersize=8)
                self.ax.set_xlim(0, 1)
                self.ax.set_ylim(0, 1)
                self.ax.set_xticks(np.linspace(0, 1, self.num_subdivisions + 1))
                self.ax.set_yticks(np.linspace(0, 1, self.num_subdivisions + 1))
                self.ax.grid(True, linestyle='--', alpha=0.7)
                self.ax.set_aspect('equal')
                self.fig.patch.set_facecolor('#f0f0f0')

                self.canvas.draw()
                self.apply_preset()
                self.status_label.config(text=f"Subdivisions Updated: {value}", foreground='green')
            else:
                messagebox.showerror("Error", "Subdivisions must be between 5 and 30.")
                self.status_label.config(text="Error: Invalid Subdivision Value", foreground='red')
        except ValueError:
            messagebox.showerror("Error", "Invalid input. Please enter an integer.")
            self.status_label.config(text="Error: Invalid Input", foreground='red')

    def snap_to_grid(self, y):
        """Snaps to grid."""
        return min(np.linspace(0, 1, self.num_subdivisions+1), key=lambda x: abs(x - y))

    def on_press(self, event):
        """Handles mouse press."""
        if event.inaxes:
            for i, (x, y) in enumerate(zip(self.x_values, self.points)):
                if abs(x - event.xdata) < 1 / (2 * self.num_subdivisions):
                    self.dragging_point = i
                    break

    def on_motion(self, event):
        """Handles mouse motion."""
        if self.dragging_point is not None and event.inaxes:
            snapped_y = self.snap_to_grid(event.ydata)
            snapped_y = max(0, min(1, snapped_y))
            self.points[self.dragging_point] = snapped_y

            global global_points
            global_points = self.points[:]

            self.line.set_ydata(self.points)
            self.canvas.draw()

    def on_release(self, event):
        """Handles mouse release."""
        self.dragging_point = None
        self.status_label.config(text="Curve Modified", foreground='blue')

    def enter_custom_curve(self):
        """Enters custom curve."""
        curve_str = simpledialog.askstring("Custom Curve", "Enter custom curve points (comma-separated, e.g., 0,0.2,0.5,1):")
        if curve_str:
            try:
                curve_values = [float(x.strip()) for x in curve_str.split(',')]
                if len(curve_values) >= 2:
                    num_points = len(curve_values)
                    num_subdivisions = num_points - 1

                    self.num_subdivisions = num_subdivisions
                    self.num_points = num_points
                    self.subdivision_entry.delete(0, tk.END)
                    self.subdivision_entry.insert(0, str(num_subdivisions))
                    self.x_values = np.linspace(0, 1, num_points)  # Keep x_values normalized
                    self.points = curve_values
                    global global_points
                    global_points = self.points[:]

                    self.ax.clear()
                    self.line, = self.ax.plot(self.x_values, self.points, marker='o', linestyle='-', markersize=8)
                    self.ax.set_xlim(0, 1)
                    self.ax.set_ylim(0, 1)
                    self.ax.set_xticks(np.linspace(0, 1, 11))
                    self.ax.set_yticks(np.linspace(0, 1, 11))
                    self.ax.grid(True, linestyle='--', alpha=0.7)
                    self.ax.set_aspect('equal')
                    self.fig.patch.set_facecolor('#f0f0f0')
                    self.canvas.draw()
                    self.preset_var.set("Custom")
                    self.status_label.config(text="Custom Curve Entered", foreground='green')

                    # Output the curve parameters in libinput-compliant format
                    step = 1 / num_subdivisions
                    output_string = "accel_profile = custom "
                    output_string += f"{step+1:d} "  # Use the calculated step
                    output_string += " ".join(f"{p:.6f}" for p in curve_values) #format the points
                    self.output_text.delete(1.0, tk.END)
                    self.output_text.insert(tk.END, output_string)

                else:
                    messagebox.showerror("Error", "Must enter at least two points.")
                    self.status_label.config(text="Error: Must enter at least two points", foreground='red')
            except ValueError as e:
                messagebox.showerror("Error", f"Invalid input: {e}")
                self.status_label.config(text="Error: Invalid Input Format", foreground='red')

if __name__ == "__main__":
    root = tk.Tk()
    app = InteractiveLineChart(root)
    root.mainloop()
    print("[ ", end="")
    print(*global_points, sep=", ", end=" ]")
